...
2. [명명 규칙 (Naming)](#2-명명-규칙-naming)
3. [코드 스타일 및 패턴](#3-코드-스타일-및-패턴)
4. [리팩토링 원칙](#4-리팩토링-원칙)
5. [협업 및 테스트 규칙](#5-협업-및-테스트-규칙)
6. [현재 코드베이스 관행 (문서 대비)](#6-현재-코드베이스-관행-문서-대비)
...
- **관심사 분리 (Suffix)**: 파일이 복잡해질 경우 다음 접미사를 활용해 파일을 세분화합니다.
  ...
  - `hooks/use-*.ts`: 상태 관리 및 비즈니스 로직 전용 (또는 기능 폴더 내 `_components/use-*/` — 현재 저장소에도 이 패턴이 있음)
...
> 일부 레거시/예외로 `*.type.ts`(단수) 접미사 파일이 존재할 수 있습니다. 신규 코드는 가급적 `*.types.ts`로 통일하는 것을 권장합니다.
...
- **함수 정의**: 특별한 제약이 없다면 **화살표 함수(`=>`)** 사용을 지향합니다.  
  (현재 저장소에는 `export function Component()`와 화살표 컴포넌트가 혼용되어 있으므로, 새 코드부터라도 통일을 권장합니다.)
- **상태 관리(Zustand)**: 불필요한 리렌더링 방지를 위해 **Selector 방식**으로 상태를 구독하는 것을 권장합니다.  
  (예: `useAuthStore((s) => s.user)` — 전체 스토어를 매번 구독하지 않기.)
- **데이터 처리(TanStack Query)**: `services/`에서 API 호출부와 Query 훅을 통합 관리하는 것을 권장합니다. (도입 시점에 맞춰 디렉터리를 정리.)
...
- **폼**: 복잡한 입력 및 유효성 검사는 **`react-hook-form` + `zod`** 활용을 권장합니다. (프로젝트에 아직 없을 수 있음 — 필요 시 추가.)
...
아래는 **가이드(권장)** 와 **현재 저장소에서 실제로 보이는 패턴**을 짧게 맞춰 둔 참고입니다. 리팩터/온보딩 시 혼동을 줄이기 위함입니다.

| 구분                  | 가이드                                     | 현재 관행                                                      |
| :-------------------- | :----------------------------------------- | :------------------------------------------------------------- |
| 폴더 구조             | `ui/` · `common/` · `app/.../_components/` | 일치. `common` 대형 기능은 내부 `_components/` 다단도 사용 중. |
| 테스트·스토리 위치    | 컴포넌트와 동일 폴더                       | 대체로 일치.                                                   |
| Barrel `index.ts`     | 권장                                       | 널리 사용 중.                                                  |
| 명명 (kebab / Pascal) | 폴더·파일 kebab, 컴포넌트 Pascal           | 대체로 일치.                                                   |
| Props 타입            | `interface` 지향                           | 다수 컴포넌트에서 `interface` 사용.                            |
| 타입 파일명           | `*.types.ts`                               | `*.types.ts`와 `*.type.ts` 혼용 — 신규는 `*.types.ts` 권장.    |
| 컴포넌트 정의         | 화살표 함수 지향                           | `export function`과 `export const` 혼용.                       |
| Zustand               | selector 구독 권장                         | 일부에서 스토어 전체 구독 — 점진적 개선 권장.                  |
| `services/` + Query   | API·훅 통합 권장                           | 도입 단계에 맞게 확장 가능.                                    |
| 폼                    | RHF + Zod 권장                             | 복잡 폼 도입 시 추가 검토.                                     |
| `*.schema.ts`         | 복잡 시 사용                               | 필요 시 추가.                                                  |

이 섹션은 저장소 상태에 따라 주기적으로 갱신하는 것이 좋습니다.
