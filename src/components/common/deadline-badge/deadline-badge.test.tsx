...
import { DeadlineBadge } from '@/components/common/deadline-badge/deadline-badge';
...
describe('DeadlineBadge', () => {
  ...
  it('마감 후에는 마감 종료가 표시된다', () => {
    render(<DeadlineBadge registrationEnd="2025-03-18T10:00:00+09:00" variant="groupEat" />);

    expect(screen.getByText('마감 종료')).toBeInTheDocument();
  });

  it('24시간 초과 남으면 일·시간 문구가 표시된다', () => {
    render(<DeadlineBadge registrationEnd="2025-03-26T10:00:00+09:00" variant="groupEat" />);

    expect(screen.getByText(/모집완료까지 7일 0시간 남았어요!/)).toBeInTheDocument();
  });

  it('24시간 이내면 시·분 문구가 표시된다', () => {
    render(<DeadlineBadge registrationEnd="2025-03-20T08:00:00+09:00" variant="groupBuy" />);
  ...
  it('groupBuy이면 buy용 알람 아이콘을 쓴다', () => {
    render(<DeadlineBadge registrationEnd="2025-03-26T10:00:00+09:00" variant="groupBuy" />);
