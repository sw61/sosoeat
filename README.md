...
소셜 로그인(카카오, 구글) 및 자체 회원가입을 지원합니다. 이메일 형식, 중복 가입, 비밀번호 규칙 등 실시간 유효성 검사를 통해 입력 오류를 즉시 안내합니다.
...
키워드 검색과 함께 카테고리 · 지역 다중 선택 · 날짜 범위 · 인기순 · 모임일 임박순 등 다양한 필터를 제공하여 원하는 모임을 빠르게 탐색할 수 있습니다.
...
카카오맵 API를 연동하여 장소를 직접 검색하고 지도에서 확인할 수 있습니다. 모임 제목 · 카테고리 · 날짜 · 인원수 등을 단계별로 입력해 누구나 간단하게 모임을 개설할 수 있습니다.
...
모임의 제목 · 날짜 · 장소 · 인원 등 상세 정보를 한눈에 확인할 수 있습니다. 참여 여부 및 모임 확정 상태에 따라 UI가 동적으로 변화하며, 원하는 댓글에 답변할 수 있는 대댓글 기능과 공유 기능을 제공합니다.
...
모임 외에도 일상을 자유롭게 공유하고 소통할 수 있는 커뮤니티 공간입니다. 굵게 · 기울임 · 밑줄 · 정렬 · 목록 · 이미지 첨부 등을 지원하는 리치 텍스트 에디터를 직접 개발하여 풍부한 글쓰기가 가능합니다.
...
프로필 이미지 크롭(Crop) 기능을 지원하며, 서비스 내 활동 내용을 수치로 확인할 수 있습니다. 모임 카드의 하트 버튼으로 찜한 모임을 저장하고, 참여한 모임 · 찜한 모임 · 내가 만든 모임을 탭으로 한눈에 관리할 수 있습니다.
